...
__kernel void app(const int N, const char dest_rank, const SMI_Comm comm)
{
    ...
    SMI_Channel chan=SMI_Open_send_channel_ad(N, SMI_DOUBLE2, dest_rank, 0, comm, 2048);
    //const double start=0.1f;
	const double2 start = (double2)(0.1f);
    for(int i=0;i<N;i++)
    {
		double2 send = start+(double2)(0.1f);
        SMI_Push(&chan,&send);
    }
}
...
__kernel void app_1(const int N, const char dest_rank, const SMI_Comm comm)
{
    ...
    SMI_Channel chan=SMI_Open_send_channel_ad(N,SMI_DOUBLE2,dest_rank,1, comm, 2048);
    //const double start=0.1f;
	const double2 start = (double2)(0.1f);
    for(int i=0;i<N;i++)
    {
